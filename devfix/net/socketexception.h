 ...
 */
	struct socketexception : public base::error::baseexception
	{
		 ...
		 */
		explicit socketexception(const std::string& what_arg, int err = -1)
			: base::error::baseexception(what_arg, err)
		{
		}
		 ...
		 * @param err c error code (errno)
		 */
		explicit socketexception(const char* what_arg, int err = -1)
			: base::error::baseexception(what_arg, err)
		{
		}
	};
